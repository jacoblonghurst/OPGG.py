...
# todo: we shouldn't be making a sqlite db to cache things. We should be using some kind of built in cache like
#  lru or use TimedCache
class Cacher:
    """
    ...
    def setup(self) -> None:
        ...
        if len(cache_db) > 0 and "-" in cache_db[0]:
            ...
            if (datetime.now() - cache_last_updated).days >= 7:
                ...
                self.logger.info(
                    "Cache has been rebuilt! The immediate request following a cache rebuild might take slightly longer as new data is fetched and the cache is updated."
                )
    ...
    def insert_summoner(self, summoner_name: str, summoner_id: str, return_result: bool = False) -> None | str:
        ...
        return_msg = (
            f"You have made changes to the database. Table: tblSummoners | Rows affected: {self.cursor.rowcount}"
        )
    ...
    def get_all_champs(self) -> list[Champion] | None:
        ...
        if result is None:
            ...
            raise Exception(
                "Multiple levels of data fetching have failed to result in this event.\n \
            ...
            for i, cached_champ in enumerate(result):
                ...
                self.logger.info(
                    f'Successfully rebuilt the "{champ_obj.name}" champion object from cache. ({i+1}/{len(result)})'
                )
    ...
    def get_passive(self, champion_id: int) -> Passive | None:
        ...
        return Passive(name=result[1], description=result[2], image_url=result[3], video_url=result[4])
    ...
    def get_all_seasons(self) -> list[SeasonInfo] | None:
        ...
        if result is None:
            ...
            for i, season in enumerate(result):
                ...
                self.logger.debug(
                    f'Successfully rebuilt the "{season_obj.display_value}" season object from cache. ({i+1}/{len(result)})'
                )
